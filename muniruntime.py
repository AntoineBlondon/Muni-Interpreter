...
declare("get_imported_files", get_imported_files)
...
def parse_import(file_path, namespace):
...
def evaluate_expression(ast, context):
    ...
    if node_type == 'binop':
        _, op, left, right = ast
        left_value = run_ast(left, context)
        right_value = run_ast(right, context)
        ...
        _, elements = ast
        list_value = [run_ast(element, context) for element in elements]
        return list_value
    elif node_type == 'identifier':
        return context.local_scope.get(ast[1], symbol_table.get(ast[1], None))['value']
    elif node_type == 'casting':
        _, expr, type_to_cast = ast
        value = run_ast(expr, context)
        value = cast(value, type_to_cast)
        return value
    

def manage_variable(ast, context):
    node_type = ast[0]

    if node_type == 'declare':
        _, var_type, var_name, elements = ast
        if var_type.startswith("LIST"):
            list_value = run_ast(elements, context)
            check_type(list_value, var_type)
            if context.is_local:
                context.local_scope[var_name] = {'type': var_type, 'value': list_value}
                
            else:
                symbol_table[var_name] = {'type': var_type, 'value': list_value}
        else:
            value = run_ast(elements, context)
        ...
        _, var_name, expr = ast
        value = run_ast(expr, context)
        var_info = context.local_scope.get(var_name, symbol_table.get(var_name, None))
        if var_info is None:
...
def control_flow(ast, context):
    node_type = ast[0]
    if node_type == 'if':
        condition, true_block = ast[1], ast[2]
        condition_value = run_ast(condition, context)
        if condition_value:
            run_ast(true_block, context)
            ...
            try:
                for element in list_value:
                    ...
                    if context.is_local:
                        ...
                        symbol_table[iter_var] = {'type': iter_type, 'value': element}
                    run_ast(body, context)
            except BreakException:
                pass
    ...
    # For your custom 'do while' loops
        ...
        if(not isinstance(num_of_iteration, int)):
            num_of_iteration = run_ast(num_of_iteration, context)
        try:
        ...
        try:
            ...
            switch_value = run_ast(switch_expr, context)
            
            executed_case = False
            for case_type, case_value, case_body, a, b in case_statements:
                
                if case_type == 'case':
                    case_eval_value = run_ast(case_value, context)
                    if switch_value == case_eval_value:
                        run_ast(case_body, context)
                        executed_case = True
                        break
...
def manage_function(ast, context):
    node_type = ast[0]
    if node_type == 'function_declaration':
        _, func_type, func_name, params, func_body, default_return = ast
        function_table[func_name] = (func_type, params, func_body, default_return)
        if(context.debug): print(f"Debug: Declaring function {func_name} with parameters {params}")
        if default_return is not None:
            check_type(run_ast(default_return, context), func_type)
        ...
        if func_name in native_functions:
            ...
            return call_native_function(func_name, args, run_ast, symbol_table, context)
        if(context.debug): print(f"Debug: Calling function {func_name} with arguments {args} and parameters {params}")
        func_type, params, func_body, default_return = function_table.get(func_name, (None, [], [], None))
        for (param_type, param_name), arg in zip(params, args):
            arg_value = run_ast(arg, context)
        ...
        new_local_scope.update({param[1]: {'type': param[0], 'value': run_ast(arg, context) }for param, arg in zip(params, args)})
        ...
        try:
            ...
            run_ast(func_body, context)
        except ReturnException as e:
            return e.value

        # If the function body didn't return, use the default return value
        return run_ast(default_return, context)

def list_operations(ast, context):
    node_type = ast[0]
    if node_type == 'index_access':
        list_ast, index_ast = ast[1], ast[2]
        list_value = run_ast(list_ast, context)
        index_value = run_ast(index_ast, context)
        if index_value < 0:
        ...
        list_value = context.local_scope.get(list_ast, symbol_table.get(list_ast, None))['value']
        index_value = run_ast(index_ast, context)
        new_value = run_ast(value_ast, context)
        ...
        index = run_ast(index_expr, context)
        value = run_ast(value_expr, context)
        list_value = context.local_scope.get(list_name, symbol_table.get(list_name, None))['value']
        ...
        list_name, value_expr = ast[1], ast[2]
        to_remove = run_ast(value_expr, context)
        list_value = context.local_scope.get(list_name, symbol_table.get(list_name, None))['value']
...
def special_constructs(ast, context):
    node_type = ast[0]
    if node_type == 'break':
        ...
        if len(ast) == 2:
            ...
            if(not isinstance(file_path, str)):
                file_path = run_ast(file_path, context)
        ...
        imported_ast = parse_import(file_path, namespace)
        global main_ast
        main_ast = merge_asts(main_ast, imported_ast)
        
        run_ast(main_ast, context)
